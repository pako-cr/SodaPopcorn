...
final class MovieDetailsVC: BaseViewController {
    ...
    private var movie: Movie? {
        didSet {
            DispatchQueue.main.async { [weak self] in
                ...
                // Title
                if let title = movie.movieTitle, !title.isEmpty, self.oldMovie?.movieTitle != title {
    ...
    private func favoriteButtonPressed() {
        if let movie = self.movie {
            viewModel.inputs.favoriteButtonPressed(movie: movie)
        }
    }
...
extension MovieDetailsVC: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollView.convert(headerStack.frame.origin, to: self.view).y <= 60 {
            navigationController?.navigationBar.topItem?.title = movie?.movieTitle ?? ""
        } else {
