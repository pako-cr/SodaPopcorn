...
final class ProfileImageViewCell: UICollectionViewCell, UIScrollViewDelegate {
    // MARK: Constants
    static let reuseIdentifier = "profileImageViewCellId"
    ...
    private var imageURL: String? {
        didSet {
            DispatchQueue.main.async { [weak self] in
                ...
                if let cacheImage = cache.value(forKey: "\(PosterSize.w185.rawValue)\(imageURL)") {
    ...
    private let profileImage = CustomProfileImage(resetImage: false)
    ...
    func setupCellView() {
        ...
        scrollContentView.addSubview(profileImage)

        profileImage.topAnchor.constraint(equalTo: scrollContentView.topAnchor).isActive = true
        profileImage.leadingAnchor.constraint(equalTo: scrollContentView.leadingAnchor).isActive = true
        profileImage.trailingAnchor.constraint(equalTo: scrollContentView.trailingAnchor).isActive = true
        profileImage.bottomAnchor.constraint(equalTo: scrollContentView.bottomAnchor).isActive = true
        profileImage.heightAnchor.constraint(equalTo: heightAnchor, multiplier: 1.0).isActive = true
    ...
    // MARK: - 🗑 Deinit
    deinit {
        //        print("🗑 ProfileImageViewCell deinit.")
    }
}
