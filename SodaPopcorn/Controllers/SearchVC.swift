    ...
    private func updateDataSource(genres: [Genre]? = nil, movies: [Movie]? = nil, animatingDifferences: Bool = true) {
        DispatchQueue.main.async { [weak self] in
            ...
            if let movies = movies, !movies.isEmpty {
                snapshot.deleteItems(snapshot.itemIdentifiers(inSection: .movies))
                snapshot.appendItems(movies.map({ SearchObject(movie: Movie(id: $0.id, movieTitle: $0.movieTitle, overview: $0.overview, rating: $0.rating, posterPath: $0.posterPath, backdropPath: $0.backdropPath, releaseDate: $0.releaseDate)) }), toSection: .movies)
            }
